	...
	<div class="app-contain" :style='{"padding":"20px 20px","margin":"20px auto 40px","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"1200px","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">比赛内容</div>
					<div  class="info_text" >{{detail.bisaineirong}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'报名')" @click="bisaibaomingonAcross('报名','bisaibaoming','是','[1]','已报名')" type="warning">报名</el-button>
					<el-button class="approval_btn" v-if="btnAuth('bisaixinxi','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('bisaixinxi','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('bisaixinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('bisaixinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<el-tabs type="border-card" v-model="activeName" class="tabs_view">
			<el-tab-pane label="比赛规则" name="first">
				<div v-html="detail.bisaiguize"></div>
			</el-tab-pane>
		</el-tabs>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.bisaimingcheng
			bannerList.value = res.data.data.bisaitupian?res.data.data.bisaitupian.split(','):[]
	...
	const bisaibaomingonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
