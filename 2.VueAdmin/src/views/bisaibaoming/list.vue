	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('bisaibaoming','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('bisaibaoming','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('bisaibaoming','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('bisaibaoming','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('bisaibaoming','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('bisaibaoming','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('bisaibaoming','参加')" type="success" @click="canjiabisaiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">参加</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<canjiabisaiFormModel ref="canjiabisaiFormModelRef" @formModelChange="formModelChange"></canjiabisaiFormModel>
	</div>
</template>
<script setup>
	...
	const approvalClick = (row) => {
		let params = {
			...
			canjiashijian: row.canjiashijian,
			xuehao: row.xuehao,
			xueshengxingming: row.xueshengxingming,
			duiwumingcheng: row.duiwumingcheng,
			sfsh: row.sfsh,
			shhf: row.shhf,
	...
	import canjiabisaiFormModel from '@/views/canjiabisai/formModel'
	const canjiabisaiFormModelRef = ref(null)
    const canjiabisaiCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			canjiabisaiFormModelRef.value.init(row.id,'cross','参加',row,'bisaibaoming',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
